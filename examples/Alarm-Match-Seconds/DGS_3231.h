...
union DS3231_TIME {
  ...
  struct __attribute__((__packed__)) {
    ...
    uint8_t a1seconds :4; // 1s of alarm 1 seconds
    uint8_t a1sec10   :3; // 10s of alarm 1 seconds
    uint8_t a1m1      :1; // alarm 1 match bit 1

    // byte 0x08 alarm 1 minutes
    uint8_t a1minutes :4; // 1s of alarm 1 minutes
    uint8_t a1min10   :3; // 10s of alarm 1 minutes
    uint8_t a1m2      :1; // alarm 1 match bit 2
    
    // byte 0x09 alarm 1 hours
    uint8_t a1hours   :4; // 1s of alarm 1 hours, 0 - 9
    uint8_t a1hour10  :1; // 10s of alarm 1 hours, 0 - 1
    uint8_t a1hr20pa  :1; // 20s of alarm 1 hours, 0 - 1, or PM/AM(low)
    uint8_t a1hr12    :1; // alarm 1 12hr/24hr(low) mode
    uint8_t a1m3      :1; // alarm 1 match bit 3
    
    // byte 0x0a alarm 1 day/date
    uint8_t a1daydate :4; // 1s of alarm 1 day or date
    uint8_t a1dat10   :2; // 10s of alarm 1 date
    uint8_t a1dydt    :1; // alarm 1 day/date(low) flag
    uint8_t a1m4      :1; // alarm 1 match bit 4
    
    // byte 0x0b alarm 2 minutes
    uint8_t a2minutes :4; // 1s of alarm 2 minutes
    uint8_t a2min10   :3; // 10s of alarm 2 minutes
    uint8_t a2m2      :1; // alarm 2 match bit 2 (there is no bit 1)
    
    // byte 0x0c alarm 2 hours
    uint8_t a2hours   :4; // 1s of alarm 2 hours, 0 - 9
    uint8_t a2hour10  :1; // 10s of alarm 2 hours, 0 - 1
    uint8_t a2hr20pa  :1; // 20s of alarm 2 hours, 0 - 1, or PM/AM(low)
    uint8_t a2hr12    :1; // alarm 2 12hr/24hr(low) mode
    uint8_t a2m3      :1; // alarm 2 match bit 3
    
    // byte 0x0d alarm 2 day/date
    uint8_t a2daydate :4; // 1s of alarm 2 day or date
    uint8_t a2dat10   :2; // 10s of alarm 2 date
    uint8_t a2dydt    :1; // alarm 2 day/date(low) flag
    ...
    int8_t tempint;       // signed, two's complement integer value

    // byte 0x12 fraction of degree of temperature
    uint8_t           :6; // reserved
    uint8_t tempfrac  :2; // 0=0, 1=0.25, 2=0.50, 3=0.75
